...
func _check_action_log(time: int):
	if _current_action_step < actions_log.size():
		if actions_log[_current_action_step][0] * _input_scaling_coeff <= time:
				print(time, ", ", actions_log[_current_action_step])
				_do_action(actions_log[_current_action_step])
